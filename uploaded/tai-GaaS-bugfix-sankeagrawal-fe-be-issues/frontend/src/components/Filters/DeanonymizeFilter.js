...
export default function DeanonymizeFilter(props) {
    const {filterName, helperText} = props;
    const [state, dispatch] = useContext(Context);

    const handleCheck = (event) => {
        dispatch({type: "DEANONYMIZE_OUTPUT", enabled: event.target.checked});
    };

    return (
        <div>
            <Grid sx={{ pt: 2, pb: 2 }} container direction="row" justifyContent="space-evenly" alignItems="center">
                <Switch checked={state.Deanonymize_OUTPUT.enabled} onChange={handleCheck}/>
