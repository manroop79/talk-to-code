...
export default function RelevanceFilter(props) {
    const {filterName, helperText} = props;
    const marks = [{value: 0, label: '0'}, {value: 1, label: '1'}];
    const [state, dispatch] = useContext(Context);

    const handleCheck = (event) => {
        dispatch({type: "Relevance_OUTPUT", enabled: event.target.checked});
    };

    const handleThreshold = (event, newValue) => {
        dispatch({type: "Relevance_OUTPUT", threshold: newValue})
    };

    return (
        <div>
            <Grid sx={{ pt: 2, pb: 2 }} container direction="row" justifyContent="space-evenly" alignItems="center">
                <Switch checked={state.Relevance_OUTPUT.enabled} onChange={handleCheck}/>
                <Accordion sx={{width: "50%"}}>
                    ...
                    <AccordionDetails>
                        ...
                        <Slider
                            value={state.Relevance_OUTPUT.threshold}
