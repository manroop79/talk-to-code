...
export default function PromptInjectionFilter(props) {
    ...
    const handleCheck = (event) => {
        dispatch({type: "PROMPT_INJECTION", enabled: event.target.checked})
    };

    const handleChange = (event, newValue) => {
        dispatch({type: "PROMPT_INJECTION", threshold: newValue})
    };

    return (
        <div>
            <Grid sx={{ pt: 2, pb: 2 }} container direction="row" justifyContent="space-evenly" alignItems="center">
                <Switch checked={state.PromptInjection.enabled} onChange={handleCheck}/>
                <Accordion sx={{width: "50%"}}>
                    ...
                    <AccordionDetails>
                        ...
                        <Slider
                            value={state.PromptInjection.threshold}
